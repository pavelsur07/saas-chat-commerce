...
trait WebFormCorsTrait
{
    private function handlePreflight(Request $request, string $publicKey, CrmWebFormRepository $forms): ?Response
    {
        ...
        if (!$forms->isStorageReady()) {
            return $this->applyCors(new JsonResponse(['error' => 'Forms storage is not ready'], Response::HTTP_SERVICE_UNAVAILABLE), $request, $fallbackOrigin);
        }

        $form = $forms->findActiveByPublicKey($publicKey);
        if (!$form) {
            return $this->applyCors(new JsonResponse(['error' => 'Form not found'], Response::HTTP_NOT_FOUND), $request, $fallbackOrigin);
        }
